...
<head>
    ...
    <title>Tabela de reajuste de produto</title>
</head>
<body>

<h1>Tabela de reajuste de produto</h1>
    <nav>
  <ul>
...
$sql = "SELECT * FROM reajusteproduto";
...
echo"<table border='3' cellpadding='8' cellspacing='0'>";
        echo'<tr>';
        echo'<th> idreajusteProduto </td>';
        echo'<th> valorCusto</td>';
        echo'<th> valorVenda </td>';
        echo'<th> preçoNovo </td>';
...
while($linha = mysqli_fetch_assoc($resultado)){
    ...
    echo'<td>' . $linha['idreajusteProduto'] . '</td>';
    echo'<td>' . $linha['valorCusto'] .'</td>';
    echo'<td>' . $linha['valorVenda'] .'</td>';
    echo'<td>' . $linha['precoNovo'] .'</td>';
    echo'<td>' . $linha['produtos_idproduto'] .'</td></tr>';
    

}
while($linha = mysqli_fetch_assoc($resultado)){
    echo $linha['idreajusteProduto'];
    echo $linha['valorCusto'];
    echo $linha['valorVenda'];
    echo $linha['precoNovo'];

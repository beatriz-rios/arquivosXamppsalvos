...
<head>
    ...
    <title>Registro das Agências</title>
    ...
    <style>
        ...
        #tabela td {
            padding: 12px 10px;
            border-bottom: 1px solid var(--premium-black); /* Linhas de separação sutis */
        ...
        #tabela tbody tr:nth-child(even) {
            background-color: #222222; /* Linha escura */
        }
        #tabela tbody tr:nth-child(odd) {
            background-color: #333333; /* Linha um pouco mais clara */
        }
...
<?php
    ...
    if (!$resultado) {
        ...
        echo '<th>CNPJ</th>';
        echo '<th>Endereço</th>';
        echo '<th>Número Agência</th>';
        echo '<th>ID Instituição</th>';
        echo '</tr>';
        echo '</thead>';
        echo '<tbody>';
        ...
        while ($linha = mysqli_fetch_assoc($resultado)) {
            echo '<tr>';
            echo '<td>' . htmlspecialchars($linha['idagencia']) . '</td>';
            echo '<td>' . htmlspecialchars($linha['cnpj']) . '</td>';
            echo '<td>' . htmlspecialchars($linha['endereco']) . '</td>';
            echo '<td>' . htmlspecialchars($linha['numero_agencia']) . '</td>';
            echo '<td>' . htmlspecialchars($linha['instituicaofinan_idinstituicao']) . '</td>';
            echo '</tr>';
        }

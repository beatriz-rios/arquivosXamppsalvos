...
<head>
    ...
    <title>Registro das Movimentações Bancárias</title>
...
<body>
    <h1>Registro das Movimentações Bancárias</h1>
...
<?php
    ...
    if (!$resultado) {
        ...
        echo '<th>Valor</th>';
        echo '<th>Débito/Crédito</th>';
        echo '<th>Data Mov.</th>';
        echo '<th>Hora Mov.</th>';
        echo '<th>ID Conta Bancária</th>';
        ...
        while ($linha = mysqli_fetch_assoc($resultado)) {
            // Formatação do valor para moeda (Exemplo: R$ 1.234,56)
            $valor_formatado = 'R$ ' . number_format($linha['valor'], 2, ',', '.');
            
            echo '<tr>';
            echo '<td>' . htmlspecialchars($linha['idmovimentacao']) . '</td>';
            echo '<td>' . htmlspecialchars($linha['descricao']) . '</td>';
            echo '<td>' . htmlspecialchars($valor_formatado) . '</td>';
            echo '<td>' . htmlspecialchars($linha['deb_cred']) . '</td>';
            echo '<td>' . htmlspecialchars($linha['data_mov']) . '</td>';
            echo '<td>' . htmlspecialchars($linha['hora_mov']) . '</td>';
            echo '<td>' . htmlspecialchars($linha['conta_bancaria_idconta_bancaria']) . '</td>';

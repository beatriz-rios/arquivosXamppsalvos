...
<head>
    ...
    <title>Acessar Agência Bancária</title>
    ...
    <style>
        ...
        form {
            ...
            max-width: 450px; /* Levemente menor para mais elegância */
...
<body>
    ...
    <form method="post" enctype="multipart/form-data">
        CNPJ da Agência:<input type="number" name="cnpj" required>
        Endereço da Agência: <input type="text" name="endereco" required>
        Número da Agência:<input type="number" name="agen" required>
        ID da Instituição:<input type="number" name="idinst" required>
        <input type="submit" value="Cadastrar Agência">
    </form>
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        // CORREÇÃO DE SEGURANÇA: Usando Prepared Statements
        // Evita SQL Injection e garante que todos os dados sejam tratados corretamente.
        $sql = " INSERT INTO agencia (
            cnpj,
            endereco,
            numero_agencia, 
            instituicaofinan_idinstituicao
            )VALUE(
            '$idpasc',
            '$nome',
            '$cpf',
        ...
        if ($stmt) {
            // "isii": i=integer (cnpj), s=string (endereco), i=integer (numero_agencia), i=integer (idinst)
            // Se o CNPJ for uma string grande, use 's'
            // Assumindo que CNPJ é um número (integer/bigint) e endereco é string
            mysqli_stmt_bind_param($stmt, "isii", $cnpj, $ende, $agen, $idinst);

            if(mysqli_stmt_execute($stmt)){
                echo"<div class='mensagem sucesso'>Comando executado com sucesso: Agência Cadastrada.</div>";
            }else{
